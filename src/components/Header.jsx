      ...
      <header className={`heading ${scrolled ? 'position-fixed' : ''}`}>
        <Container className='d-flex align-items-center justify-content-between'>
          <a className="logo" to="/">
            ...
            <img src={logoSm} className='d-sm-none' alt="" />
          </a>
            ...
            <ul className="d-lg-flex align-items-center">
              {navLinks.map((link, index) => (
                <li key={index} className='d-block'>
                  <Link spy={true} smooth={true} duration={100} offset={-100} className="heading-link text-capitalize" to={link.to} onClick={() => handleTouchStart(link.to)}>{link.btn_title}</Link>
                </li>
              ))}
              <li className='heading-actions d-grid align-items-center d-lg-none'>
                <a to='/sign-up' className='btn bg-primary text-white'>Contact Us</a>
              </li>
            </ul>
          </nav>
          <div className="heading-actions d-flex align-items-center flex-wrap">
            <a to='/sign-up' className='btn d-none d-sm-inline-block bg-primary text-white'>Contact Us</a>
